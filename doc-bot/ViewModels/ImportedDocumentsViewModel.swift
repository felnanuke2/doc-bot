...
class ImportedDocumentsViewModel: ObservableObject {
    ...
    private lazy var importedDocumentsRepository: any ImportedDocumentRepository = Container.shared.importedDocumentRepository()
    ...
    init() {
        Task {
            let docs = await try! importedDocumentsRepository.list()
...
//        let now = Date()
        ...
        try! await importedDocumentsRepository.create(entity: importedDocument)
